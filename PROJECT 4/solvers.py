...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for param, grad in zip(param_vars, grad_tensors):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        "*** YOUR CODE HERE ***"
        for param, grad, velocity in zip(param_vars, grad_tensors, vel_vars):
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        batch_size= 1
        batch_train= MinibatchIndefinitelyGenerator(train_data, batch_size, self.shuffle)
        batch_val= MinibatchIndefinitelyGenerator(val_data, batch_size, self.shuffle)
        ...
        for iter_ in xrange(self.iterations):
            ...
            """
            once_train_data, once_target_data= batch_train.next()
            once_val_train_data, once_val_target_data= batch_val.next()
            
            train_loss= session.run(loss_tensor, feed_dict={placeholders[0]: once_train_data, placeholders[1]: once_target_data})
            session.run(update_ops, feed_dict={placeholders[0]: once_train_data, placeholders[1]: once_target_data})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: once_val_train_data, placeholders[1]: once_val_target_data})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        batch_train= MinibatchIndefinitelyGenerator( train_data, self.batch_size, self.shuffle )
        batch_val= MinibatchIndefinitelyGenerator( val_data  , self.batch_size, self.shuffle )
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            #util.raiseNotDefined()
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            # val_loss should be the loss of this iteration using the same amount of data used for the updates, but using the validation data instead
            batch_train_data, batch_target_data= batch_train.next()
            batch_val_train_data, batch_val_target_data= batch_val.next()
            
            train_loss= session.run(loss_tensor, feed_dict={placeholders[0]: batch_train_data, placeholders[1]: batch_target_data})
            session.run(update_ops, feed_dict={placeholders[0]: batch_train_data, placeholders[1]: batch_target_data})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_val_train_data, placeholders[1]: batch_val_target_data})
